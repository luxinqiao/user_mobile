...
<div id='main' :class='data.is_authed=="0"?(isIphoneX?"hasBtnIphoneX":"hasBtn"):""' :style='"padding-top:"+headHeight+"px"'>
    ...
    <div id='banner'>
        <md-video :videoSrc='banner.videoSrc' :tryTime='banner.tryTime'
            ...
            <div class='banner-shade preview' v-if='data.is_preview=="1"'>
            ...
            <div class='banner-shade lockBuy' v-if='banner.coverShade=="lockBuy"'>
                <p class='text'>请先购买专栏</p>
            </div>
        </md-video>
    </div>
    <div id='info'>
        <div class='info-title'>
            <img class='info-title-logo' :src='require("@/assets/img/college/spec/detail/info-logo.png")'>
        ...
        <div class='info-study'>
            <span class='info-study-num'>{{data.status=="0"?data.sub:data.learn_num}}</span>
            <span class='info-study-unit'>{{data.status=="0"?"人预约":"人学习"}}</span>
        </div>
    ...
    <div id='tabHead'>
        <div class='tabHead-content' ref='tabHead'>
            ...
            <div :class='tab=="catalog"?"tabHead-content-item active":"tabHead-content-item"' @click='tab="catalog"'>
    ...
    <div id='tabContent'>
        ...
        <div id='catalog' v-if='tab=="catalog"'>
            <div class='catalog-title'>更新至第{{data.lesson_count}}课/共{{data.lesson_total}}课</div>
            <div class='catalog-item' :class='i==catalogActiveNo?"active":""' v-for='(item,i) in data.list' :key='i'>
                <md-open-xcx v-if='checkOpenXcxAuth(item)' :domId='"catalog"+i' :canUseWx='xcxOpen.canUse' :extinfo='xcxOpen.param' 
                    :xcxType='xcxOpen.type' :xcxPath='xcxOpen.path+param.spe_code' @callbackFn='cancelLastWatch(i)'>
                    <div class='catalog-item-box'>
                        <img class='mark' :src='getCatalogSource(item.source_type)'>
                        <p class='text text_ellipsis2'>{{i+1}}. {{item.name}}</p>
                        <p class='liveText' v-if='item.source_type=="1" && data.is_authed=="1"'>
                            <img class='liveText-type' v-if='item.is_broad=="1"' :src='require("@/assets/img/college/spec/detail/course-video.png")'>
                            <img class='liveText-type' v-else :src='require("@/assets/img/college/spec/detail/course-audio.png")'>
                            <span class='liveText-status'>{{getLiveStatus(item.live_status)}}</span>
                            <span class='liveText-time'>{{item.start_at}}</span>
                        </p>
                        <img class='btn-pre' v-if='item.is_preview=="1"' :src='require("@/assets/img/college/spec/detail/catalog-pre.png")'>
                        <span class='btn-try' v-else-if='data.is_authed=="0"&&item.is_try_see=="1"'>可试看</span>
                        <img class='btn-lock' v-else-if='data.is_authed=="0"' :src='require("@/assets/img/college/spec/detail/course-lock.png")'>
                        <span class='btn-recent' v-else-if='item.is_last_see=="1" && pageDir == "forward"'>上次观看</span>
                    </div>
                </md-open-xcx>
                <div v-else class='catalog-item-box' @click='playCatalog(item,i)'>
                    <img class='mark' :src='getCatalogSource(item.source_type)'>
                    <p class='text text_ellipsis2'>{{i+1}}. {{item.name}}</p>
                    <p class='liveText' v-if='item.source_type=="1" && data.is_authed=="1"'>
                        <img class='liveText-type' v-if='item.is_broad=="1"' :src='require("@/assets/img/college/spec/detail/course-video.png")'>
            ...
            <div class='catalog-nomore'>
                {{data.lesson_count==data.lesson_total?"-没有更多内容-":"-更新中，敬请期待-"}}
            </div>
        </div>
        <div id='comment' v-if='tab=="comment"'>
            ...
            <div class='comment-content' v-if='comment.num!="0"'>
                <div class='comment-item' v-for='(pItem, pI) in comment.datas' :key='pI'>
                    ...
                    <div v-if='pItem.reply_count>0' class='answer-box'>
    ...
    <div id='btn' v-if='data.is_authed=="0"' :class='isIphoneX?"iphoneX_btn":""'>
        <div class='btn-try' v-if='data.is_try_see=="1"' @click='tryWatch()'>
...
<script lang='ts'>
    ...
    import {mdPopup, mdVideo, mdToast, mdHead, mdOpenApp, mdOpenXcx, mdCustom} from '@/components'
    import {getBrowserType, getToken, priceTransition, priceTransitionYuan, convertRem, getHeadHeight, convertPx, limit, isEmpty, checkIOSX} from '@/utils/common'
    ...
    @Component({
        components: {
            mdPopup, mdVideo, mdToast, mdHead, mdOpenApp, mdOpenXcx, mdCustom
        }
    })

    export default class collegeSpecDetail extends Vue {
        private func: object = {
            priceTransition,
            priceTransitionYuan,
            getAppProduct
        ...
        private param: any = {
            spe_code: this.$route.query.spe_code
        }
        private data: any = {}
        ...
        private tab: string = 'catalog'
        private catalogActiveNo: number = -1
        private comment: any = {
            datas: [],
            num: '',
            pageNo: 1,
        ...
        private pageDir: string = 'forward'
        ...
        activated() {
            ...
            this.initCoupon()
            this.initComment()
            this.pageDir = sessionStorage.routeDirection
        }
         ...
         */
        initData() {
            this.$axios.post(`${this.$request.live}/v3/source/spe`, {
                spe_code: this.param.spe_code
            }).then((res: any) => {
        ...
        initLastWatch() {
            for (let i=0; i<this.data.list.length; i++) {
                ...
                if (item.is_preview == '1') { //预告
                    continue
                }
                if (this.data.is_authed=="0" && item.is_try_see == '1') { //试看
                    continue
                }
                if (this.data.is_authed == '0') { //锁（未授权）
                    continue
                }
                if (item.is_last_see == '1') { //上次观看
                    this.catalogActiveNo = i
                    return
                }
            }
        }
        ...
        initCoupon() {
            this.$axios.post(`${this.$request.live}/v1/coupon/canget`, {
                where_code: this.param.spe_code,
                for_where: '3' //专栏
            }).then((res: any) => {
        ...
        initScrollListener() {
            ...
            this.$nextTick(() => {
                const mdVideoEl = (this.$refs.mdVideo as any).$el
                ...
                window.onscroll = (e: any) => {
                    ...
                    if (scrollTop >= originMdVideoTop - this.headHeight) {
                        mdVideoEl.style = `position:fixed;z-index:98;top:${this.headHeight}px;left:0;right:0;`
                    } else {
                        mdVideoEl.style = null
                    }
        ...
        buy() {
            checkToken(()=>{
                ...
                url += `?type=3&token=${getToken()}&code=${this.param.spe_code}&price=${this.data.price}&detail_url=${encodeURIComponent(window.location.href)}`
        ...
        tryWatch() {
            ...
            for (let i=0; i<this.data.list.length; i++) {
                ...
                if (item.is_preview == '0' && item.is_try_see == '1') {
                    this.playCatalog(item, i)
                    return
                }
            }
        }
        ...
        reWatch() {
            ...
            let item = this.data.list[this.catalogActiveNo]
            this.playCatalog(item, this.catalogActiveNo)
        }

        /*
            播放目录（播放视频/跳转直播）
            @param item 目录项：课程/直播
            @param i 目录序号
            @return
         */
        playCatalog(item: any, i: number) {
            //预告 -> 登录 -> 试看课程 -> 锁（购买）-> 正常（课程：播放；直播：跳详情/跳小程序）
            if (item.is_preview == '1') { //预告
                this.toast.show = true
                if (item.source_type == '0') { //课程
                    this.toast.content = `该课程预计${item.release_time}发布`
                } else { //直播
                    this.toast.content = `该直播预计${item.release_time}发布`
                }
                return
            }
            ...
            if (item.source_type == '0' && item.is_try_see == '1') { //课程+试看
                this.cancelLastWatch(i)
                this.catalogActiveNo = i
                this.playVideo(item.code)
                return
            }
            if (this.data.is_authed == 0) { //锁（未授权）
                this.toast.content = '请先购买专栏'
                this.toast.show = true
                this.banner.coverShade = 'lockBuy'
                ;(this.$refs.mdVideo as any).showCover('mode')
                return
            }
            ...
            this.cancelLastWatch(i)
            this.catalogActiveNo = i
            if (item.source_type == '1') { //直播
                this.$router.push(`../live/detail?live_code=${item.code}&type=${item.is_broad == "1" ? "LT13" : "LT11"}`)
            } else { //课程
                this.playVideo(item.code)
            }
        }
        ...
        /*
            ...
            @return
         */
        cancelLastWatch(i: number) {
            if (this.data.list[i].source_type == '1') { //直播
                (this.$refs.mdVideo as any).end()
                ;(this.$refs.mdVideo as any).showCover('mode')
            }
            if (this.catalogActiveNo == i) {
                return
            }
            if (this.catalogActiveNo != -1) {
                this.data.list[this.catalogActiveNo].is_last_see = '0'
            }
            this.catalogActiveNo = i
            this.$axios.post(`${this.$request.live}/jzxy/source/spe-last-see`, {
                spe_code: this.param.spe_code,
                child_code: this.data.list[i].code
            }).then((res: any) => {
            })
        }
        ...
        /*
            ...
            @return
         */
        playVideo: Function = limit(function(cancelLimit: Function, code: string) {
            this.videoTime = 0
            ;(this.$refs.mdVideo as any).hideCover()
            this.$axios.post(`${this.$request.live}/v1/vod/common`, {
                ...
                if (res.code == 200) {
                    ...
                    this.banner.tryTime = parseInt(this.data.list[this.catalogActiveNo].free_duration)
        ...
        addLearn: Function = limit(function(cancelLimit: Function) {
            ...
            this.$axios.post(`${this.$request.live}/v2/learn/add`, {
                source_code: this.param.spe_code,
                source_type: '1', //0-课程,1-专栏,2-直播
                rote_code: this.catalogActiveNo + 1,
                rote_time: this.videoTime,
                chap_code: this.data.list[this.catalogActiveNo].code,
                isTourist: true
            }).then((res: any) => {
                cancelLimit()
            })
        ...
        onVideoEnded(time: number) {
            this.addLearn()
            for (let i=this.catalogActiveNo+1; i<this.data.list.length; i++) {
                let catalogItem = this.data.list[i]
                if (catalogItem.source_type == '1') { //直播
                    continue
                } else { //课程
                    if (catalogItem.is_preview == '1') { //预告
                        continue
                    }
                    if (this.data.is_authed == '0') { //锁（未授权）
                        continue
                    ...
                    this.cancelLastWatch(i)
                    this.catalogActiveNo = i
                    this.playVideo(catalogItem.code)
                    break
                }
            }
        }
        ...
        /**
            ...
            @return 打开小程序权限
         */
        checkOpenXcxAuth(item: any) {
            if (this.browserType != "wechat") { //app+浏览器
                return false
            }
            if (this.data.is_preview == 1) { //预告
                return false
            }
            if (this.data.is_authed == 0) { //未授权
                return false
            }
            if (item.source_type == "0") { //课程
                return false
            }
            if (item.is_broad == "1") { //视频直播
                return false
            }
            return true
        }
        ...
        /*
            获取课程类型
            @param source_type 课程类型（0-课程 1-直播）
            @return 课程类型
         */
        getCatalogSource(source_type: string) {
            if (source_type == '0') {
                return require("@/assets/img/college/spec/detail/catalog-course.png")
            }
            return require("@/assets/img/college/spec/detail/catalog-live.png")
        }

        /*
            获取直播状态
            @param live_status 直播状态（0-未开始 1-进行中 2-已结束）
            @return 直播状态
         */
        getLiveStatus(live_status: string) {
            if (live_status == '0') {
                return '未开始'
